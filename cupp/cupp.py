...
def read_config(filename):
    ...
    if os.path.isfile(filename):
        ...
        print("dosya " + filename + " bulunamadı!")
        sys.exit("çıkılıyor.")
...
def print_to_file(filename, unique_list_finished):
    ...
    f.close()
    print(
        "[+] dosya şu isimle kaydediliyor "
        + filename
        + " tam "
        + str(lines)
        + " kelime."
    )
    inspect = input("> Hyperspeed ister misiniz? (Y/n) : ").lower()
    if inspect == "y":
        try:
    ...
    print(
        "[+] şimdi dosyanız yükleniyor  "
        + filename
        + " ve program kapatılıyor görüşürüz.iyi şanslar"
    )


def print_cow():
    print('       __                   _ ___________ ')
    print('      |  |                /  |           \ ')
    print('      |  |               /   |            \ ')
    print('      |  |              (    |             \ ')
    print('      |  |          __  \    |              )')
...
def improve_dictionary(file_to_open):
    ...
    if conts == "y" and len(listic) > CONFIG["global"]["threshold"]:
        print(
            "\r\n[-] birleştirmenin maksimum numarası "
            + str(CONFIG["global"]["threshold"])
        )
        print("[-] Bu sayıyı artırmak için CONFIG dosyasını kontrol edin.\r\n")
        conts = input(
            "> kelime listesindeki tüm kelimeleri birleştirmek ister misiniz ? Y/[N]: "
        ).lower()
    ...
    spechars = [""]
    spechars1 = input(
        "> kelimelerin sonuna özel karakter koymak ister misiniz? Y/[N]: "
    ).lower()
    if spechars1 == "y":
        for spec1 in chars:
    ...
    randnum = input(
        "> kelimelerin sonuna numaralar koymak ister misiniz ? Y/[N]:"
    ).lower()
    leetmode = input("> Leet mod ister misiniz? (i.e. leet = 1337) Y/[N]: ").lower()
    ...
    print("\r\n[+] şimdi bir wordlist dosyası oluşturuluyor ...")
...
def interactive():
    ...
    print("\r\n[+] hedef hakkında bir kaç bilgi verin ki wordlist oluşturalım")
    print("[+] eğer sorulan bilgiyi bilmiyorsanız sorulunca boş bırakın! ;)\r\n")


    profile = {}

    name = input("> hedef adı : ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] bir isim girin!")
        name = input("> hedef adı : ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> hedef soyadı: ").lower()
    profile["nick"] = input("> takma adı : ").lower()
    birthdate = input("> doğum tarihi (GGAAYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] doğum tarihi sekiz karakter olmalı!")
        birthdate = input("> doğum tarihi (GGAAYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> eşinin adı: ").lower()
    profile["wifen"] = input("> eşinin takma adı : ").lower()
    wifeb = input("> eşinin doğum tarihi (GGAAYYYY): ")
    while len(wifeb) != 0 and len(wifeb) != 8:
        print("\r\n[-] doğum tarihi için 8 karakter girmelisiniz!")
        wifeb = input("> eşinin doğum tarihi (GGAAYYYY): ")
    profile["wifeb"] = str(wifeb)
    print("\r\n")

    profile["kid"] = input("> çocuğunun adı : ").lower()
    profile["kidn"] = input("> çocuğunun soyadı : ").lower()
    kidb = input("> çocuğunun doğum tarihi (GGAAYYYY): ")
    while len(kidb) != 0 and len(kidb) != 8:
        print("\r\n[-] doğum günü için 8 karakter gireceksin!")
        kidb = input("> çocuğunun doğum günü (GGAAYYYY): ")
    profile["kidb"] = str(kidb)
    print("\r\n")

    profile["pet"] = input("> evcil hayvanının adı : ").lower()
    profile["company"] = input("> şirketinin adı : ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> acaba hedefin şifresinde kullanmış olabileceği kelimeler girmek ister misiniz? Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "> kelimeleri girin.virgülle ayırabilirsiniz: "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> acaba kelimelerin sonuna özel karakter eklensin mi? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> peki kelimelerin sonuna rastgele sayı eklemek ister misiniz? Y/[N]:"
    ).lower()
    profile["leetmode"] = input("> Leet mod ister misiniz? (i.e. leet = 1337) Y/[N]: ").lower()
...
def generate_wordlist_from_profile(profile):
    ...
    print("\r\n[+] şimdi bir wordlist oluşturuluyor...")
...
def download_http(url, targetfile):
    print("[+] yükleniyor " + targetfile + " nereden :  " + url + " ... ")
...
def download_wordlist():
    ...
    print("	\r\n	indirmek istediğiniz wordlistin numarasını girin:\r\n")
    ...
    print(
        "	\r\n	dosyalar şuradan indiriliyor :  "
        + CONFIG["global"]["dicturl"]
        + " depo"
    )
    print(
        "	\r\n	bilgi : wordlisti indirdikten sonra -w ile onu kullanabilirsiniz\r\n"
    )

    filedown = input("> numara girin: ")
    filedown.isdigit()
    while filedown.isdigit() == 0:
        print("\r\n[-] yanlış.böyle bir numara yok. ")
        filedown = input("> numara girin: ")
...
def download_wordlist_http(filedown):
    ...
    if intfiledown in arguments:
        ...
        print("[+] dosyalar şuraya kaydedildi :  " + dire)
